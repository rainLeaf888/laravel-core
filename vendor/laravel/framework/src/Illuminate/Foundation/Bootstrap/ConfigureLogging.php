...
class ConfigureLogging
{
     ...
     * @return void
     */
    protected function configureCrmHandler(Application $app,Writer $log)
    {
        $logPath = $app->make('config')->get('crm.logPath.crmLog');
        if ($logPath == '') {
            $logPath = $app->storagePath(). '/logs/';
        }
        $log->useCrmFiles(
            $logPath . '/' . $app->make('config')->get('app.log_prefix').'.log' ,
            $app->make('config')->get('app.log_max_files',5)
        );
    }
